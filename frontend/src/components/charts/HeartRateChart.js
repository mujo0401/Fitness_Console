...
const DiagnosticsDialog = ({ open, onClose, tokenScopes, isAuthenticated, date, period }) => {
    ...
    >
      <DialogTitle sx={{ 
        ...
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
          ...
          <Typography variant="h6">Heart Rate Data Diagnostics</Typography>
        </Box>
...
const HeartRateChart = ({ data, period, tokenScopes = [], isAuthenticated = false, date = new Date() }) => {
  const theme = useTheme();
